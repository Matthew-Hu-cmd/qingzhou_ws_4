	...
	if len(contours) > 0:
		...
		if cx == 0 and cy == 0:
			...
			if text == 'left':
				...
				shift = (cx - midpoint)/80
				if shift < -22:
					shift = -22     # 限制右转角度，右转角度太大会擦到车道线
		# print(shift)
		return shift
...
if __name__ =='__main__':
	...
	cmd_vel_pub = rospy.Publisher('/ackermann_cmd', Twist, queue_size=10)
	...
	try:
		...
		while not rospy.is_shutdown():
			...
			if shift:
				twist_contrl.linear.x = 1.3
				twist_contrl.angular.z = shift
				# old_angle = angle
				# ark_contrl.steering_angle_velocity = 6.0
				print(shift)
				cmd_vel_pub.publish(twist_contrl)
				out_check = 0
			else:
				out_check += 1
			...
			# 出弯判断
			if out_check > 10:
				twist_contrl.linear.x = 0
				twist_contrl.angular.z = 0
				cmd_vel_pub.publish(twist_contrl)
				print('out, stop')
				out_check = 0
				break
