...
from robot_vision.cfg import VisionConfig
import cv2
...
class RoadLine:
	def __init__(self, video):
		self.Kp, self.Kd, self.Speed = 1.0, 0.22, 1.1
		self.video = video
		self.ark_contrl = AckermannDrive()
		self.cmd_vel_pub = rospy.Publisher('/ackermann_cmd', AckermannDrive, queue_size=1)
		self.locate_pub = rospy.Publisher("/qingzhou_locate", Int32, queue_size=1)
		self.param_server = Server(VisionConfig, self.reconfigure)

	def reconfigure(self, config, level):
		print(config)
		self.Kp, self.Kd, self.Speed = config['Kp'], config['Kd'], config['Speed']
		rospy.loginfo("Reconfigure request :   Kp: %f Kd: %f Speed: %f" % (config['Kp'], config['Kd'], config['Speed']))    
	...
	def roadline(self):
		...
		while self.video.isOpened():
			...
			if ret is None:
				...
				# 出弯判断
				if out_check > 8:
					self.ark_contrl.speed = 0
					self.ark_contrl.steering_angle = 0
					self.cmd_vel_pub.publish(self.ark_contrl)
					print('out, stop')
					D_error = [0, 0, 0]
				...
				if shift:     # PID控制角度
					...
					# i=0
					# while i<2:
					# 	D_error[i+1] = D_error[i]
					# 	i+=1
					# D_error[0] = now_err-last_err
					D_error = D_error[1:]
					D_error.append(now_err - last_err)
					now_angle = self.Kp * now_err + self.Kd * (D_error[2] * 0.6 + D_error[1] * 0.3 + D_error[0] * 0.1)
				else:
					out_check += 1
				...
				# else:
				angle = -now_angle
				self.ark_contrl.speed = self.Speed
				self.ark_contrl.steering_angle = angle
				print(f"angle:{angle}")
				self.cmd_vel_pub.publish(self.ark_contrl)
				
		self.locate_pub.publish(5)
...
if __name__ =='__main__':
	...
	rospy.init_node("vision_dynamic_server")
