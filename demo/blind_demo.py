...
class navigation_demo:
    ...
    def robot_goal_update(self):    #实现在不同的位置区域发布不同的目标点
		#判断小车所处的区域
        ...
        # 这里需不需要手动延时来拉慢发布频率，怕算不过来
        if(self.j == 0):
            # rospy.loginfo("Location 1")
            w = [pdX[self.j+1],pdY[self.j+1],pdth[self.j+1]]
            nav.goto([w[0],w[1],w[2]])
        elif(self.j==1):
            w = [pdX[self.j+1],pdY[self.j+1],pdth[self.j+1]]
            nav.goto([w[0],w[1],w[2]])
        elif(self.j==2):
...
if __name__ == "__main__":
    ...
    pdX = [1.966, 2.034, -2.213, 0.796, 0]
    pdY = [-4.244, -6.200, -5.414, -3.929, 0]
    pdth = [0,0,0,0,0]
    length = len(pdX)
    try:
        nav = navigation_demo()     
